...
def parte_a():
    ...
    # Que la portada declare el curso y el centro
    p = paginas_html.get("/", "")
    for tipo in ('"Course"', '"EducationalOrganization"'):
        if tipo not in p:
    ...
    # El formulario
    for url in ("/", "/contacto/", "/curso-reciclaje-metal-4-horas/",
                "/oficios/", "/tarjeta-profesional-metal/"):
